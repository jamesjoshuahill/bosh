  ...
  describe Api::Controllers::ConfigsController do
    ...
    describe 'diff' do
      ...
      context 'authenticated access' do
        ...
        context 'when the given id is not an integer' do
          before do
            allow(Bosh::Director::Models::Config).to receive(:[]).with('foo').and_raise(Sequel::DatabaseError)
            allow(Bosh::Director::Models::Config).to receive(:[]).with('1').and_return(double())
          end
          ...
          it 'returns 400 with error details' do
            ...
            expect(last_response.status).to eq(400)
            expect(JSON.parse(last_response.body)['code']).to eq(440010)
            expect(JSON.parse(last_response.body)['description']).to eq("Config with ID 'foo' not found.")
